...
Data lake:
- Generally contains unstructured data.
- Stores huge amounts of data, sometimes in terabytes and petabytes.
- Use cases: stream processing, machine learning and real-time analytics.
- Target users: data scientists or  data analyst

Data warehouse:
- Generally contains structured data.
- Data size, generally small.
...
- Is a solution for data warehouse.
- Schema on write: define schema, define relationship and write data

ELT (Extract, Load & Transform):
- Used for large amounts of data.
- Is a solution for data lake.
...
In previous week's lecture we completed a data ingestion <a href="https://github.com/Balurc/data_eng_zoomcamp/blob/main/week1_introduction/2_docker_sql/ingest_data.">process/script</a> that downloads NYC LTC dataset (parquet) and store it out Postgres database. While this process is simple and able to deliver some results, but this is a less than ideal choice of data ingestion process. 
...
- Upload and download large files from a storage service.


Follow these steps to start ingesting data to GCP.
- In your `airflow/dags` folder create a pyhton file `data_ingestion_gcs_dag.py`. This file/scripts will contain our ingestion process/instructions.
- Import the neccesary libraries as below:
...
- Next, create 2 variables based on environment variable that we set in our `docker-compose.yml` file:
...
- Create python function to upload data to GCS.
...
- The create the default arguments and dag declaration as below
    default_args = {
        "owner": "airflow",
...
- You can see the full implementation here. Please note that in this ingestion we only have 3 tasks, not 4 as what is shown in the course videos and repo. This is because the current dataset (NYC TLC) is already in parquet format, hence we do not need the task that converts our data from csv to parquet file format.

- Once the script is ready, you first need to ensure that your containers are running (`docker-compose ps`), if they are not, you can follow the previous steps in the earlier section.

- Now go to localhost:8080, your airflow UI and start the data ingestion process, click on `data_ingestion_gcs_dag` and press the play button on your top right, then click on `Trigger DAG` to start. Turn on the `auto-fresh` button so that we can see the status and progress of our dag and tasks.
<insert image>
<insert image>

- Once every task has completed successfully (as shown above), go to GCP and search for BigQuery. You will find the data that we just ingested under `trips_data_all` dataset in `external_table` table. Then, go to your GCS and under `raw` folder you will see the parquet file that we just ingested.
...
My solution for this week's homework is mostly based on the solution given by the instructor but with some customizations as below. You can check the full implementation here.
...
Its 12 run. Because our dag run once per month.
